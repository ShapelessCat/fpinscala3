...
enum Option[+A] {
  ...
  infix def flatMap_1[B](f: A => Option[B]): Option[B] = this match {
    case None    => None
    case Some(a) => f(a)
  }
  ...
  infix def orElse_1[B >: A](ob: => Option[B]): Option[B] = this match {
    case None => ob
    case _    => this
  }
...
object Option {
  ...
  // It can also be implemented using `foldRight` and `map2`. The type annotation on `foldRight` is
  // needed here; otherwise Scala wrongly infers the result type of the fold as `Some[Nil.type]` and
