...
trait Parsers[Parser[+_]] { self =>  // so inner classes may call methods of trait
  ...
  given Conversion[String, Parser[String]] =
    string(_)

  given operators[A]: Conversion[Parser[A], ParserOps[A]] =
    ParserOps[A](_)

  given asStringParser[A](using f: A => Parser[String]): Conversion[A, ParserOps[String]] =
    a => ParserOps(f(a))
  ...
  case class ParserOps[A](p: Parser[A]) {
    ...
    infix def or[B >: A](p2: => Parser[B]): Parser[B] =
      self.or(p, p2)
    ...
    infix def flatMap[B](f: A => Parser[B]): Parser[B] =
      self.flatMap(p)(f)
...
case class ParseError(stack: List[(Location, String)] = Nil) {
  ...
  */
  ...
  /** Builds a collapsed version of the given error stack - messages at the same location have their
   *  messages merged, separated by semicolons
   */
