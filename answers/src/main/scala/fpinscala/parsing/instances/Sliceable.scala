...
object SliceableTypes {
   ...
   *  This type is slightly fancier than the one discussed in the chapter, to support efficient
   *  slicing. If the parser is surrounded by a `slice` combinator, the `isSliced` field of
   *  `ParseState` will be `true`, and we return a `Slice` output.
   */
  ...
  case class ParseState(loc: Location, isSliced: Boolean) {
    ...
    def input: String =
      loc.input.substring(loc.offset)

    def unslice: ParseState =
      copy(isSliced = false)

    def reslice(s: ParseState): ParseState =
      copy(isSliced = s.isSliced)
...
object Sliceable extends Parsers[Parser] {
  ...
  /** See this gist for more information, examples, and discussion of Scala's GADT support:
  ...
  /** We provide an overridden version of `many` that accumulates the list of results using a
   *  monolithic loop. This avoids stack overflow errors.
   */
