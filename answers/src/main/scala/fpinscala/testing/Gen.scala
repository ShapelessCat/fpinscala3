...
object Prop {
  ...
  def forAll[A](g: Int => Gen[A])(f: A => Boolean): Prop = Prop {
    (max, n, rng) =>
      ...
      val props: LazyList[Prop] = LazyList.from(0).take((n min max) + 1).map { i =>
        forAll(g(i))(f)
      }
      val prop: Prop =
        props.map { p =>
...
case class Gen[+A](sample: State[RNG, A]) {
  ...
  infix def flatMap[B](f: A => Gen[B]): Gen[B] = Gen {
    sample.flatMap { a =>
      f(a).sample
    }
  }
...
object Gen {
  ...
  def choose(start: Int, stopExclusive: Int): Gen[Int] = Gen {
    State(RNG.nonNegativeInt).map { n =>
  ...
  /** Basic idea is to add 1 to the result of `choose` if it is of the wrong parity, but we require
   *  some special handling to deal with the maximum integer in the range.
   */
  ...
  /** A `Gen[Par[Int]]` generated from a list summation that spawns a new parallel computation for
   *  each element of the input list summed to produce the final result. This is not the most
   *  compelling example, but it provides at least some variation in structure to use for testing.
   *
   *  Note that this has to be a `lazy val` because of the way Scala initializes objects. It depends
   *  on the `Prop` companion object being created, which references `pint2`.
   */
