 ...
 *
 *  In the implementation of `flatMap`, rather than calling the function, we throw an exception
 *  indicating what function we want to call. A central loop repeatedly tries and catches these
 *  exceptions to force the computation.
 */
enum Throw[+A] {
  case Done(a: A)
...
object Throw extends Monad[Throw] {
  ...
  def ap[A, B](a: A)(f: A => B): B = {
    ...
    while (true) {
      try return fi(ai).asInstanceOf[B]
        ...
        case Call(a2, f2) =>
          ai = a2
          fi = f2.asInstanceOf[Any => Any]  // TODO: ???
      }
    }
  ...
  def more[A](a: => Throw[A]): Throw[A] =
    More(() => a)

  /** `Throw` forms a `Monad`. */

  def unit[A](a: => A): Throw[A] =
    more(Done(a))
