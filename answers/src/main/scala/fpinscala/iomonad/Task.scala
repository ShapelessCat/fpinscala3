...
/** `Task[A]` is a wrapper around `Free[Par, Either[Throwable, A]]`, with some convenience functions
 *  for handling exceptions.
 */
case class Task[A](get: IO[Either[Throwable, A]]) {

  infix def flatMap[B](f: A => Task[B]): Task[B] = Task(
    get.flatMap {
  ...
  def or[B >: A](t2: Task[B]): Task[B] = Task(
    this.get flatMap {
      case Left(e) => t2.get
      case a       => IO(a)
    }
  ...
  def run(using E: ExecutorService): A = unsafePerformIO(get) match {
    case Left(e)  => throw e
    case Right(a) => a
  }
...
object Task extends Monad[Task] {
  ...
  def more[A](a: => Task[A]): Task[A] =
    Task.now(()) flatMap (_ => a)

  def delay[A](a: => A): Task[A] =
    more(now(a))

  def fork[A](a: => Task[A]): Task[A] = Task {
    par { Par.lazyUnit(()) } flatMap (_ => a.get)
  }

  def forkUnit[A](a: => A): Task[A] =
    fork(now(a))
