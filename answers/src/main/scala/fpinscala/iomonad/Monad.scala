...
trait Monad[F[_]] extends Functor[F] {
  ...
  def sequence_[A](fs: F[A]*): F[Unit] =
    sequence_(fs.to(LazyList))
  ...
  // syntax
  given toMonadic[A]: Conversion[F[A], Monadic[F, A]] =
    a => new Monadic[F, A] {
...
trait Monadic[F[_], A] {
  ...
  def get: F[A]
  private val a = get
  infix def map[B](f: A => B): F[B] = F.map(a)(f)
  infix def flatMap[B](f: A => F[B]): F[B] = F.flatMap(a)(f)
