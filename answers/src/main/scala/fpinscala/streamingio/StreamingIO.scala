...
import fpinscala.iomonad.api.{Free, IO, unsafePerformIO}
...
object ImperativeAndLazyIO {
  ...
  /** We are going to consider various approaches to the simple task of checking whether a file
   *  contains more than 40,000 lines.
   *
  ...
  def linesGt40k(filename: String): IO[Boolean] = IO {
    ...
    Using.resource(io.Source.fromFile(filename)) { src =>
      var count = 0
   ...
   *  * Find a line index before 40,000 where the first letter of consecutive lines
   *    spells out `"abracadabra"`.
   *
  ...
  object Examples {
    ...
    val ex2: Boolean = lines.filter(_.trim.nonEmpty).zipWithIndex.exists(_._2 + 1 >= 40000)
    val ex3: Int = lines.take(40000).map(_.head).indexOfSlice("abracadabra".toList)
  }

  /** Could we actually write the above? Not quite. We could 'cheat' and return an
   *  `IO[LazyList[String]]` representing the lines of a file:
   */
  ...
  /** This is called _lazy I/O_, and it's problematic for a number of reasons, discussed in the book
   *  text. However, it would be nice to recover the same high-level, compositional style we are
   *  used to from our use of `List` and `LazyList`.
   */
}

object SimpleStreamTransducers {

  /** We now introduce a type, `Process`, representing pure, single-input stream transducers. It can
   *  be in of three states - it can be emitting a value to the output (`Emit`), reading a value
   *  from its input (`Await`) or signaling termination via `Halt`.
   */
  ...
  enum Process[I, O] {
    ...
    infix def map[O2](f: O => O2): Process[I, O2] = this match {
      case Halt()      => Halt()
    ...
    infix def flatMap[O2](f: O => Process[I, O2]): Process[I, O2] = this match {
      case Halt()      => Halt()
    ...
    /** Feed `in` to this `Process`.
     *  Uses a tail recursive loop as long as `this` is in the `Await` state.
     */
    ...
    /** See `Process.lift` for a typical repeating `Process` definition expressed with explicit
     *  recursion.
     */

    /** `Process` definitions can often be expressed without explicit recursion, by repeating some
     *   simpler `Process` forever.
     */
    ...
    /** As an example of `repeat`, see `Process.filter`. We define a convenience function here for
     *  composing this `Process` with a `Process` that filters the output type `O`.
     */
    def filter(f: O => Boolean): Process[I, O] =
      this |> Process.filter(f)

    /** Exercise 7: see definition below. */
    infix def zip[O2](p: Process[I, O2]): Process[I, (O, O2)] =
      Process.zip(this, p)
  ...
  object Process {
    ...
    def monad[I]: Monad[[X] =>> Process[I, X]] =
      new Monad[[X] =>> Process[I, X]] {
        ...
        infix def flatMap[O, O2](p: Process[I, O])(f: O => Process[I, O2]): Process[I, O2] =
          p flatMap f
      }

    // enable monadic syntax for `Process` type
    given toMonadic[I, O]: Conversion[Process[I, O], Monadic[[X] =>> Process[I, X], O]] =
      monad[I].toMonadic(_)
    ...
    /** Here's a typical `Process` definition that requires tracking some piece of state (in this
     *  case, the running total):
     */
     ...
     *
     *  Here's one implementation, with three stages - we map all inputs to 1.0, compute a running
     *  sum, then finally convert the output back to `Int`. The three stages will be interleaved -
     *  as soon  as the first element is examined, it will be converted to 1.0, then added to the
     *  running total, and then this running total will be converted back to `Int`, then the
     *  `Process` will examine the next element, and so on.
     */
    ...
    /** Exercise 7:
     *  Can you think of a generic combinator that would allow for the definition of `mean` in terms
     *  of `sum` and `count`?
     *
     *  Yes, it is `zip`, which feeds the same input to two processes.
     *  The implementation is a bit tricky, as we have to make sure that input gets fed to both `p1`
     *  and `p2`.
     */
    ...
    def processFile[A, B](f: java.io.File,
      ...
      Using.resource(io.Source.fromFile(f)) { s =>
        go(s.getLines(), p, z)
      }
    }
...
object GeneralizedStreamTransducers {
   ...
   *  The `Halt` constructor now has a _reason_ for termination, which may be either normal
   *  termination indicated by the special exception `End`, forceful terimation, indicated by the
   *  special exception `Kill`, or some other error.

   *  We'll use the improved `Await` and `Halt` cases together to ensure that all resources get
   *  released, even in the event of exceptions.
   */
  ...
  enum Process[F[_], O] {
    ...
    /** Many of the same operations can be defined for this generalized `Process` type, regardless
     *  of the choice of `F`.
     */

    infix def map[O2](f: O => O2): Process[F, O2] = this match {
    ...
    infix def onComplete(p: => Process[F, O]): Process[F, O] =
      this.onHalt {
     ...
     *  Unlike the simple `runLog` interpreter defined in the companion object below, this is not
     *  tail recursive and responsibility for stack safety is placed on the `Monad` instance.
     */
    ...
    /** We define `Process1` as a type alias - see the companion object for `Process` below. Using
     *  that, we can then define `|>` once more. The definition is extremely similar to our previous
     *  definition. We again use the helper function, `feed`, to take care of the case where `this`
     *  is emitting values while `p2` is awaiting these values.
     *
     *  The one subtlety is we make sure that if `p2` halts, we `kill` this process, giving it a
     *  chance to run any cleanup actions (like closing file handles, etc).
     */
    def |>[O2](p2: Process1[O, O2]): Process[F, O2] = {
      (p2: @unchecked) match {  // TODO: from Jian -- in the original code, there is no `@unchecked` and no `: Is[O]#f[Any]` in below
        case Halt(e)                        => this.kill onHalt { e2 => Halt(e) ++ Halt(e2) }
        case Emit(h, t)                     => Emit(h, this |> t)
        case Await(req: Is[O]#f[Any], recv) => this match {
    ...
    /** Use a `Tee` to interleave or combine the outputs of `this` and `p2`. This can be used for
     *  zipping, interleaving, and so forth. Nothing requires that the `Tee` read elements from each
     *  `Process` in lockstep. It could read fifty elements from one side, then two elements from
     *  the other, then combine or interleave these values in some way, etc.
     *
     *  This definition uses two helper functions, `feedL` and `feedR`, which feed the `Tee` in a
     *  tail-recursive loop as long as it is awaiting input.
     */
    def tee[O2, O3](p2: Process[F, O2])(t: Tee[O, O2, O3]): Process[F, O3] = {
      (t: @unchecked) match {  // TODO: from Jian -- in the original code, there is no `@unchecked` and no `: Is[O]#f[Any]` in below
        case Halt(e)                            => this.kill onComplete p2.kill onComplete Halt(e)
        case Emit(h, t)                         => Emit(h, this.tee(p2)(t))
        case Await(side: T[O, O2]#f[Any], recv) => side.get match {
    ...
    infix def zipWith[O2, O3](p2: Process[F, O2])(f: (O, O2) => O3): Process[F, O3] =
  ...
  object Process {
    ...
    /** Safely produce `p`, or run `cleanup` and halt gracefully with the exception thrown
     *  while evaluating `p`.
     */
    ...
    def runLog[O](src: Process[IO, O]): IO[IndexedSeq[O]] = IO {
      ...
      def go(cur: Process[IO, O], acc: IndexedSeq[O]): IndexedSeq[O] =
        cur match {
          ...
          case Await(req, recv) =>
            val next =
              try recv(Right(fpinscala.iomonad.api.unsafePerformIO(req)(using E)))
     ...
     *  of `to` in `Process` for an example of how to feed a `Process` to a `Sink`.
     */
...
@main def processTest(): Unit = {
  import GeneralizedStreamTransducers.*
  import Process.*
