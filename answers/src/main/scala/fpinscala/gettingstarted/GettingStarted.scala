...
object FormatAbsAndFactorial {
  ...
  // Now we can use our general `formatResult` function with both `abs` and `factorial`.
  def main(args: Array[String]): Unit = {
...
// Functions get passed around so often in FP that it's convenient to have syntax for constructing a
// function *without* having to give it a name.
