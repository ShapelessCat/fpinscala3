// Exercise 7:
// Can you think of a generic combinator that would allow for the definition of
// `mean` in terms of `sum` and `count`?
//
// Yes, it is `zip`, which feeds the same input to two processes.
// The implementation is a bit tricky, as we have to make sure that input gets
// fed to both `p1` and `p2`.
