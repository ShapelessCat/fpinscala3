 ...
 *  in chapter 5, we don't normally do this). So long as the buffer is allocated internal to the
 *  function, the mutation is not observable and RT is preserved.
 *
 *  Another common convention is to accumulate the output list in reverse order, then reverse it at
 *  the end, which doesn't require even local mutation. We'll write a reverse function later in this
 *  chapter.
*/
