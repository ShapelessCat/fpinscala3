// We'll just give a sketch here. The basic idea is to add an additional field
// to `ParseError`
...
// We also can expose combinators for selecting which error(s) get reported in
// the event that a chain of `a | b | c` fails--we might choose to collect up
// all the errors for each of the three parsers, or perhaps only show the parser
// that got the furthest in the input before failing, etc
