...
// `foldRight` function as we've done in this chapter. (We'll revisit this in a
// later chapter, when we discuss laziness).
//
// The other implementations build up a chain of functions which, when called,
// results in the operations being performed with the correct associativity. We
// are calling `foldRight` with the `B` type being instantiated to `B => B`,
// then calling the built up function with the `z` argument. Try expanding the
// definitions by substituting equals for equals using a simple example, like
// `foldLeft(List(1, 2, 3), 0)(_ + _)` if this isn't clear. Note these
// implementations are more of theoretical interest - they aren't stack-safe and
...
def foldLeftViaFoldRight_1[A, B](as: List[A], outerIdent: B)(combiner: (B, A) => B): B = {
  ...
  // For each item in the 'as' list (the 'a' parameter below), make a new delay
  // function which will use the combiner function (passed in above) when it is
  // evaluated later.  Each new function becomes the input to the previous
  // function (`delayFunc`).
  //
  ...
  // Pass the original list 'as', plus the simple identity function and the new
  // combinerDelayer to foldRight.  This will create the functions for delayed
  // evaluation with an combiner inside each one, but will not apply any of
  // those functions.
