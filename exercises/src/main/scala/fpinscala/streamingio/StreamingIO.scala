...
object ImperativeAndLazyIO {
  ...
  object Examples {
    ...
    val ex2 = lines.filter(_.trim.nonEmpty).zipWithIndex.exists(_._2 + 1 >= 40000)
...
object SimpleStreamTransducers {
  ...
  sealed trait Process[I, O] {
    ...
    /** Feed `in` to this `Process`. Uses a tail recursive loop as long as `this` is in the `Await`
     *  state.
     */
  ...
  object Process {
    ...
    /** We can convert any function `f: I => O` to a `Process[I,O]`. We simply `Await`, then `Emit`
     *  the value received, transformed by `f`.
     */
    ...
    /** Exercise 9:
     *  Write a program that reads degrees fahrenheit as `Double` values from a file, converts each
     *  temperature to celsius, and writes results to another file.
     */
...
object GeneralizedStreamTransducers {
  ...
  trait Process[F[_], O] {
    ...
    /** Many of the same operations can be defined for this generalized `Process` type,
  ...
  object Process {
    ...
    /** Helper function to safely produce `p`, or gracefully halt with an error if an exception is
     *  thrown.
     */
    ...
    /** Safely produce `p`, or run `cleanup` or `fallback` if an exception occurs
     *  while evaluating `p`.
     */
    ...
    /** Here is a simple tail recursive function to collect all the output of a `Process[IO, O]`.
     *  Notice we are using the fact that `IO` can be `run` to produce either a result or an
     *  exception.
     */
    def runLog[O](src: Process[IO, O]): IO[IndexedSeq[O]] = IO {
      ...
      def go(cur: Process[IO, O], acc: IndexedSeq[O]): IndexedSeq[O] =
        cur match {
          ...
          case Await(req, recv) =>
            val next =
              try recv(Right(fpinscala.iomonad.unsafePerformIO(req)(using E)))
    ...
    /** Generic combinator for producing a `Process[IO,O]` from some effectful `O` source. The
     *  source is tied to some resource, `R` (like a file handle) that we want to ensure is
     *  released.
    ...
    /** Create a `Process[IO,O]` from the lines of a file, using the `resource` combinator above to
     *  ensure the file is closed when processing the stream of lines is finished.
     */
    ...
    /** We now have nice, resource safe effectful sources, but we don't have any way to transform
     *  them or filter them. Luckily we can still represent the single-input `Process` type we
     *  introduced earlier, which we'll now call `Process1`.
     */
    ...
    def await1[I, O](recv: I => Process1[I, O],
                     fallback: => Process1[I, O] = halt1[I, O]): Process1[I, O] = Await(
      Get[I],
    ...
    /** An example use of the combinators we have so far: incrementally convert the lines of a file
     ...
     *  As an example, this program reads a list of filenames to process _from another file_,
     *  opening each file, processing it and closing it promptly.
     */
    ...
    /** We can attach filters or other transformations at any point in the program,
     *  for example: */
