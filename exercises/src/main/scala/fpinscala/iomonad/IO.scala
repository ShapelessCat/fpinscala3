...
object IO0 {
  // Our first attempt at data type for representing computations that may perform I/O. Has a simple
  // 'interpreter' baked in--the `run` function, which just returns `Unit`.
  trait IO { self =>
    def run(): Unit
    def ++(io: IO): IO = new IO {
...
object IO1 {
  ...
  sealed trait IO[A] { self =>
    ...
    infix def map[B](f: A => B): IO[B] = new IO[B] {
      def run = f(self.run)
    }

    infix def flatMap[B](f: A => IO[B]): IO[B] = new IO[B] {
...
object IO2a {
  ...
  object IO extends Monad[IO] {  // Notice that none of these operations DO anything
    ...
    infix def flatMap[A, B](a: IO[A])(f: A => IO[B]): IO[B] =
      a flatMap f
...
object IO2b {
  ...
  object TailRec extends Monad[TailRec] {
    ...
    infix def flatMap[A, B](a: TailRec[A])(f: A => TailRec[B]): TailRec[B] =
      a flatMap f
...
object IO3 {
  ...
  /** The `runConsoleFunction0` implementation is unfortunately not stack safe, because it relies of
   *  the stack safety of the underlying monad, and the `Function0` monad we gave is not stack safe.
   *  To see the problem, try running: `freeMonad.forever(Console.printLn("Hello"))`.
   */
