...
object Par {
  ...
  def map2[A, B, C](a: Par[A], b: Par[B])(f: (A, B) => C): Par[C] =
    (es: ExecutorService) => {
      val af = a(es) 
      val bf = b(es)
      // This implementation of `map2` does _not_ respect timeouts, and eagerly waits for the
      // returned futures. This means that even if you have passed in "forked" arguments, using this
      // map2 on them will make them wait. It simply passes the `ExecutorService` on to both `Par`
      // values, waits for the results of the Futures `af` and `bf`, applies `f` to them, and wraps
      // them in a `UnitFuture`. In order to respect timeouts, we'd need a new `Future`
      // implementation that records the amount of time spent evaluating `af`, then subtracts that
